...
export function PageBadge({ page, totalPages }: { page: number; totalPages: number }) {
  return (
    <span className="fixed right-4 top-16 z-10 rounded-full bg-primary px-3 py-1.5 font-heading text-sm font-bold text-on-primary shadow-[0_4px_12px_rgba(79,70,229,0.35)]">
      {page}/{totalPages}
    </span>
  );
}
